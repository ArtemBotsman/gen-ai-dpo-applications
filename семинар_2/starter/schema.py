"""Схема заявки на курс ДПО.

LLM умеет вернуть красивый JSON, но это ещё не значит, что данные нормальные.
Поэтому здесь описаны не только типы, но и базовые проверки: город должен быть
из нашего списка, а возраст, стаж и год выпуска не должны противоречить друг
другу.
"""
...
# Счётчик оставил специально для выводов: так видно, ловили ли валидаторы
# реальные ошибки модели, а не просто лежали в коде для галочки.
...
class Address(BaseModel):
    ...
    def city_must_be_in_list(cls, value: str) -> str:
        # Город контролирую отдельно, потому что это поле сильно влияет
        # на проверку mode collapse и итоговую гистограмму.
...
class Application(BaseModel):
    ...
    def graduation_year_must_match_age(cls, value: int, info: ValidationInfo) -> int:
        ...
        # Диапазон широкий: бывают бакалавриат, специалитет, магистратура
        # и второе образование. Главное — отсеять совсем невозможные анкеты.
