...
# ===========================================================================
# 5. Сравнение метрики между двумя периодами
# ===========================================================================
...
def _parse_period(period: str) -> dict:
    ...
    if not m:
        return {"error": f"неверный формат периода: {period!r}, ожидается YYYY-MM или YYYY-MM-DD"}
    year, month = int(m.group(1)), int(m.group(2))
    day = int(m.group(3)) if m.group(3) else 1
    if not (1 <= month <= 12):
        return {"error": f"месяц вне 1..12: {period}"}
    try:
        d = _date(year, month, day)
    except ValueError as e:
        return {"error": str(e)}
    return {"iso_date": d.isoformat(), "year": year, "month": month}


def _metric_value(metric: str, iso_date: str, year: int, month: int) -> dict:
    """Одно значение метрики на период."""
    if metric == "key_rate":
        raw = get_key_rate(on_date=iso_date)
        if "error" in raw:
            return raw
        return {"date": raw.get("date", iso_date), "value": raw["rate"], "source": raw["source"]}

    if metric.startswith("fx_"):
        currency = metric.split("_", 1)[1]
        raw = get_fx_rate(currency=currency, on_date=iso_date)
        if "error" in raw:
            return raw
        return {"date": raw.get("date", iso_date), "value": raw["rate"], "source": raw["source"]}

    if metric == "cpi":
        raw = get_inflation(year, month)
        if "error" in raw:
            return raw
        return {
    ...
    if metric == "unemployment":
        raw = get_unemployment(year, month)
        if "error" in raw:
            return raw
        return {
            "date": f"{year}-{month:02d}",
            "value": raw["unemployment"],
            "source": raw["source"],
        }

    return {
        "error": (
            f"неизвестная метрика {metric!r}. "
            "Допустимо: key_rate, fx_USD, fx_EUR, fx_CNY, cpi, unemployment"
        )
    }
...
def compare_periods(metric: str, period_a: str, period_b: str) -> dict:
    ...
    Returns:
        metric, a, b, delta, ratio, source
    """
    pa = _parse_period(period_a)
    if "error" in pa:
        return pa
    pb = _parse_period(period_b)
    if "error" in pb:
        return pb

    a = _metric_value(metric, pa["iso_date"], pa["year"], pa["month"])
    if "error" in a:
        return {"metric": metric, "error": f"период A: {a['error']}"}
    b = _metric_value(metric, pb["iso_date"], pb["year"], pb["month"])
    if "error" in b:
        return {"metric": metric, "error": f"период B: {b['error']}"}
