...
ПРАВИЛА:
1. Любая арифметика (разность, отношение, сумма, произведение) — ОТДЕЛЬНЫЙ
   подвопрос с expected_tools=["calculate"] и depends_on на подвопросы с числами.
   Исполнитель НЕ должен считать в уме.
2. Если подвопрос N зависит от ответа подвопроса K — поставь K в depends_on.
3. Для вопросов про «последний доступный период» — сначала узнай доступный месяц
   через get_inflation, потом используй его в следующих шагах.
4. Если задача не решается имеющимися tools — верни reasoning с объяснением
   и subquestions=[]. НЕ выдумывай get_cumulative_inflation, compare_periods и прочие.
5. Не добавляй в expected_tools ничего кроме четырёх перечисленных инструментов.
